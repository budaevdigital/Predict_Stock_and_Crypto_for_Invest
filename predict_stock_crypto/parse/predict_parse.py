...
from parse.user_agents_for_parse import (get_random_header_user_agent 
                                    as head_user_agent)
...
def get_html_from_url_response(url: str):
    ...
    if response.status_code != 200:
        settings.logging.error(f'Ошибка в ({response}) - Проблема с доступностью сайта') 
...
def get_predict_price_1_to_14_days(name_crypto: str) -> str:
    ...
    for count in range(0, 14):
        ...
        if predict_price is None:
            settings.logging.error(f'Ошибка в ({predict_price}) при парсинге предсказания с сайта walletinvestor') 
        ...
        all_predict_dict += str(date_predict) + ' | ' + str(format(float(predict_price), '.2f')) + ' $ \n'
