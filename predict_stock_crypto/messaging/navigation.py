...
keyboard_back_home = ReplyKeyboardMarkup(keyboard=([[btn_back_to_main]]), resize_keyboard=True)

# Главное меню
btn_predict = KeyboardButton('💰 Прогноз')
btn_watch_list = KeyboardButton('📋 Список избранного')
keyboard_main_menu = ReplyKeyboardMarkup(keyboard=([[btn_predict, btn_watch_list]]), resize_keyboard=True)

# Меню "Прогноз"
btn_info_money = KeyboardButton('🤑 Цены')
btn_graph_crypto = KeyboardButton('📈 График уровней')
keyboard_other_predict_menu = ReplyKeyboardMarkup(keyboard=([[btn_info_money, btn_graph_crypto], [btn_back_to_main]]), resize_keyboard=True)

# Меню "Избранное"
btn_add_to_watch = KeyboardButton('✅ Добавить')
btn_delete_to_watch = KeyboardButton('❌ Удалить')
keyboard_other_watchlist_menu = ReplyKeyboardMarkup(keyboard=([[btn_add_to_watch, btn_delete_to_watch], [btn_back_to_main]]), resize_keyboard=True)

# Меню "График уровней"
btn_low_level = KeyboardButton('🔴 Low')
btn_high_level = KeyboardButton('🟢 High')
keyboard_other_level_menu = ReplyKeyboardMarkup(keyboard=([[btn_low_level, btn_high_level], [btn_back_to_main]]), resize_keyboard=True)
...
def button_from_crypto_symbol_for_delete(list_cryptos: list):
    ...
    if len(list_cryptos) != 0:
        ...
        keyboard_for_delete_menu = ReplyKeyboardMarkup(keyboard=(button), resize_keyboard=True)    
