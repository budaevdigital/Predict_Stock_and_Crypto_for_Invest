...
def first_starting_messaging(update, context) -> None:
    """
    При первом общении с пользователем (нажатии /start), отсылает приветственное 
    сообщение и создаёт нового пользователя в БД
    {'username': 'dmitrybudaev', 'type': 'private', 'id': 286003757, 'last_name': 'Budaev', 'first_name': 'Dmitry'}
    """
    ...
    try:
        ...
        settings.logging.error(f'Ошибка ({error}) при создании пользователя из ТГ бота') 
    ...
    context.bot.send_message(
        ...
        text=(
            ...
            '\n'
            '[📋 <b><i>Список избранного</i></b>]: редактирование списка криптовалют.'),
...
def see_menu_predict(update, context) -> None:
    ...
    context.bot.send_message(
        ...
        text=(
            ...
            '\n'
            '[📈 <b><i>График уровней</i></b>]: графики <b>Low</b> и <b>High</b> уровней.'),
        reply_markup=button,
        parse_mode='HTML')
...
def see_menu_watchlist(update, context) -> None:
    ...
    try:
        ...
        settings.logging.error(f'Ошибка ({error}) при чтении избранных криптавалют из ТГ') 
    ...
    match cryptos_in_watchlist:
        ...
        case _:
            for row in range(len(cryptos_in_watchlist)):
                count += 1
                message = message + '\n' + str(count) + ' ' + str(cryptos_in_watchlist[row][2])
    context.bot.send_message(
        chat_id=user_chat_info.id,
        text=(
            ...
            '\n'
            '[❌ <b><i>Удалить</i></b>]: удалить криптовалюту из списка наблюдения.'),
        reply_markup=button,
        parse_mode='HTML')
    ...
    try:
        ...
        match cryptos_in_watchlist:
            case False:
                cryptos_in_watchlist = []
                button = buttons.keyboard_other_watchlist_menu
                msg = 'Список избранного пуст.\n\n[✅ Добавить]: добавить криптовалюту на наблюдение.'
            case _:
                button = buttons.button_from_crypto_symbol_for_delete(cryptos_in_watchlist)
                msg = 'Выберите криптовалюту, которую нужно убрать из списка наблюдения 🔽'
    except Exception as error:
        settings.logging.error(f'Ошибка ({error}) при показе избранных криптавалют в ТГ') 
    context.bot.send_message(
        chat_id=user_chat_info.id,
        text=(
...
def choise_and_delete_crypto_from_watchlist(update, context) -> None:
    ...
    try:
        ...
        settings.logging.error(f'Ошибка ({error}) при удалении избранных криптавалют из ТГ') 
    cryptos_in_watchlist = base.reading_crypto_in_watchlist(user_chat_info.id)
    match cryptos_in_watchlist:
        case False:
            cryptos_in_watchlist = []            
    ...
    context.bot.send_message(
        ...
        text=(
            ...
            '\n'
            f'Доступны на наблюдение <b>{watchlist_position - len(cryptos_in_watchlist)}</b> позиций'
            '\n'
            '\n'
            '[✅ <b><i>Добавить</i></b>]: добавить криптовалюту на наблюдение.'
            '\n'
...
def see_menu_for_add_crypto(update, context) -> None:
    ...
    context.bot.send_message(
        ...
        text=(
            ...
            '------------'        
            '\n'
            'Введите символ криптовалюты (в ответном сообщении), которую нужно добавить в список наблюдения 🔽'
            '\n'
            '<i>Пример:</i> <b>BTC</b>'),
...
def add_crypto_in_watchlist(update, context) -> None:
    ...
    try:
        match cryptos_in_watchlist:
            case False:
                cryptos_in_watchlist = []
                is_add = base.add_new_crypto_in_db_for_watch(user_chat_info.id, crypto_symbol)
                if is_add:
                    msg = f'Криптовалюта <b>{crypto_symbol}</b> добавлена в список наблюдения!'
                else:
                    msg = (f'Не удалось добавить валюту (<b>{crypto_symbol}</b>).'
            ...
            case _:
                if len(cryptos_in_watchlist) < watchlist_position:
                    ...
                    msg = 'Список избранного заполнен.\n\nУдалите, что-нибудь, перед добавлением!'   
    except Exception as error:
        settings.logging.error(f'Ошибка ({error}) при добавлении криптавалют в ТГ')           
    button = buttons.keyboard_other_watchlist_menu
    context.bot.send_message(
        chat_id=user_chat_info.id,
        text=(f'{msg}'),
    ...
    try:
        match cryptos_in_watchlist:
            case False:
                ...
                context.bot.send_message(chat_id=user_chat_info.id,
                        text=message, reply_markup=button, parse_mode='HTML')
            case _:
                for count in range(len(cryptos_in_watchlist)):
                    crypto_name = base.search_existing_field_in_db(db_name='cryptos', 
                                                column_name='symbol', 
                                                search_field=cryptos_in_watchlist[count][2])
                    crypros_predict_price = get_predict_price_1_to_14_days(crypto_name[1])
                    crypros_today_price = get_price_from_binance(symbol=cryptos_in_watchlist[count][2])
                    message = (
                        f'Прогноз для <b>{crypto_name[1]}</b> на ближайшие 2 недели:'
                        '\n'
                        '------------'        
                        '\n'            
        ...
        settings.logging.error(f'Ошибка ({error}) при запросе предсказания цен криптавалют в ТГ') 
...
def get_high_level_graph(update, context, name_levels='High', func_message='Верхних уровней') -> None:
    user_chat_info = update.effective_chat
    button = buttons.keyboard_other_level_menu
    try:
        dict_date = date_time.set_current_to_from_date()
    except Exception as error:
        settings.logging.error(f'Ошибка ({error}) при запросе даты при отрисовки графиков в ТГ') 
    cryptos_in_watchlist = base.reading_crypto_in_watchlist(user_chat_info.id)
    match cryptos_in_watchlist:
        case False:
            message = ('\nСписок избранных пуст!'
            ...
            try:
                for count in range(len(cryptos_in_watchlist)):
                    ...
                    context.bot.send_message(
                        chat_id=user_chat_info.id,
                        text=(
                            f'График {func_message} поддержки у <b>{cryptos_in_watchlist[count][2]}</b>:'
                            '\n'),
                        reply_markup=button,
                        parse_mode='HTML')
...
def get_low_level_graph(update, context) -> None:
    get_high_level_graph(update=update, context=context, name_levels='Low', func_message='Нижних уровней')
