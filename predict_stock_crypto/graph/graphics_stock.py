...
def EMA_or_SMA(moving_average: str, days_first: int,
    ...
    match moving_average:
        case 'EMA':
            ...
            df_days_first['Close Price'] = df['Close'].rolling(
                window=days_first).mean()
            df_days_second['Close Price'] = df['Close'].rolling(
                window=days_second).mean()
            return df_days_first['Close Price'], df_days_second['Close Price']
        case 'SMA':
            df_days_first = pd.DataFrame()
            df_days_second = pd.DataFrame()
            df_days_first['Close Price'] = df['Close'].ewm(
                span=days_first).mean()
            df_days_second['Close Price'] = df['Close'].ewm(
                span=days_second).mean()
            return df_days_first['Close Price'], df_days_second['Close Price']
...
def stock_to_graph(df, from_date, today_date, stock: str,
                   function: str, days_first: int = 30,
                   days_second: int = 60) -> str:
    '''
